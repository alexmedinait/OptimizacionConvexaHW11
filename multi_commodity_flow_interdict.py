...
class MultiCommodityInterdiction:
    ...
    def createPrimal(self):  
        ...
        def flow_bal_rule(model, n,k):
            ...
            successors = tmp.loc[ tmp.StartNode == n, 'EndNode'].values
            predecessors = tmp.loc[ tmp.EndNode == n, 'StartNode'].values 
    ...
    def solve(self, tee=False):
        solver = pyomo.opt.SolverFactory('cplex_direct')
        ...
        for e in self.arc_data.index:
            self.arc_commodity_data.loc[e,'xbar'] = self.Idual.x[e].value
...
if __name__ == '__main__':
    m = MultiCommodityInterdiction('sample_nodes_data-mcf.csv', 'sample_nodes_commodity_data.csv', 'sample_arcs_data-mcf.csv', 'sample_arcs_commodity_data.csv')
    m.solve()
    m.printSolution()
    m.attacks = 1
    m.solve()
